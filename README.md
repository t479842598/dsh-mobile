...
- **会话配置**：切换当前会话的模型、思考等级。
- **全局默认配置**：查看新会话默认使用的 Agent 预设、权限、模型和思考等级（修改默认配置请到 WebUI 操作）。
...
| --- | --- |
| 工作区列表 | 浏览、创建、切换工作区 |
| 会话列表 | 按最近活动排序，显示标题、运行状态、未读标记 |
| 实时流式对话 | Markdown 渲染、思考过程、工具调用/结果、图片附件 |
| 历史分页 | 向前加载更早记录，尾部事件独立更新 |
| 发送消息 | 文字 + 图片，自动创建新会话 |
| 提问应答 | Agent 提问/审批的 UI 交互与提交 |
| 会话取消 | 停止当前回合 |
| 模型切换 | 浏览并选择模型/思考等级 |
| 会话搜索 | 按内容搜索会话 |
...
部署默认配置写操作（默认 Agent/模型/权限的修改）、权限预设切换、队列编辑、Fork/归档、子代理与会话导出——这些能力 App 会提示「请前往 WebUI 操作」，不会崩溃。
